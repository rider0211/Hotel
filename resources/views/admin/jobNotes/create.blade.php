...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.jobNote.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-notes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="jnote">{{ trans('cruds.jobNote.fields.jnote') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('jnote') ? 'is-invalid' : '' }}" name="jnote" id="jnote">{!! old('jnote') !!}</textarea>
                @if($errors->has('jnote'))
