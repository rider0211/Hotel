...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.vendorFile.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vendor-files.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="vfile_name">{{ trans('cruds.vendorFile.fields.vfile_name') }}</label>
                <input class="form-control {{ $errors->has('vfile_name') ? 'is-invalid' : '' }}" type="text" name="vfile_name" id="vfile_name" value="{{ old('vfile_name', '') }}" required>
