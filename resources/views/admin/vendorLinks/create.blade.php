...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.vendorLink.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vendor-links.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="ven_link">{{ trans('cruds.vendorLink.fields.ven_link') }}</label>
                <input class="form-control {{ $errors->has('ven_link') ? 'is-invalid' : '' }}" type="text" name="ven_link" id="ven_link" value="{{ old('ven_link', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('vlink_type') ? 'is-invalid' : '' }}" name="vlink_type" id="vlink_type" required>
                    ...
                    @foreach(App\Models\VendorLink::VLINK_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('vlink_type', 'Home Site') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
