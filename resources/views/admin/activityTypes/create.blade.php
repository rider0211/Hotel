...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.activityType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.activity-types.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="act_type">{{ trans('cruds.activityType.fields.act_type') }}</label>
                <input class="form-control {{ $errors->has('act_type') ? 'is-invalid' : '' }}" type="text" name="act_type" id="act_type" value="{{ old('act_type', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('due_by') ? 'is-invalid' : '' }}" type="text" name="due_by" id="due_by" value="{{ old('due_by') }}" required>
