...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lead-files.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="jfile_title">{{ trans('cruds.leadFile.fields.jfile_title') }}</label>
                <input class="form-control {{ $errors->has('jfile_title') ? 'is-invalid' : '' }}" type="text" name="jfile_title" id="jfile_title" value="{{ old('jfile_title', '') }}" required>
