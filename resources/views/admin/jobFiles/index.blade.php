...
@can('job_file_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.job-files.create') }}">
                ...
                <tbody>
                    @foreach($jobFiles as $key => $jobFile)
                        <tr data-entry-id="{{ $jobFile->id }}">
                            ...
                            <td>
                                @can('job_file_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.job-files.show', $jobFile->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('job_file_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.job-files.edit', $jobFile->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('job_file_delete')
                                    <form action="{{ route('admin.job-files.destroy', $jobFile->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('job_file_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.job-files.massDestroy') }}",
