...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.leadNote.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lead-notes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="lead_note">{{ trans('cruds.leadNote.fields.lead_note') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('lead_note') ? 'is-invalid' : '' }}" name="lead_note" id="lead_note">{!! old('lead_note') !!}</textarea>
