        ...
        <div class="col-md-12">
            @can('work_order_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.work-orders.create') }}">
                            ...
                            <tbody>
                                @foreach($workOrders as $key => $workOrder)
                                    <tr data-entry-id="{{ $workOrder->id }}">
                                        ...
                                        <td>
                                            @can('work_order_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.work-orders.show', $workOrder->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('work_order_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.work-orders.edit', $workOrder->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('work_order_delete')
                                                <form action="{{ route('frontend.work-orders.destroy', $workOrder->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('work_order_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.work-orders.massDestroy') }}",
